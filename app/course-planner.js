...
export default function CoursePlanner() {
  ...
  return (
    <div className="max-w-5xl   rounded-2xl mx-auto p-8 relative min-h-screen">
      <h1 className="text-5xl font-extrabold mb-8 text-center text-purple-800 drop-shadow-lg tracking-tight">Course Planner</h1>
      <input
        type="text"
        placeholder="Search course code or title..."
        className="w-full mb-8 p-3 border-2 border-blue-200 rounded-xl shadow focus:outline-none focus:border-blue-400 bg-white text-lg"
        value={search}
      ...
      <div className="flex gap-10 items-start">
        <div className="grid gap-8 w-2/3">
          {filteredCourses.length === 0 && (
            <div className="text-center text-gray-400 text-lg">No courses found.</div>
          )}
          {filteredCourses.map((course) => (
            <div key={course.code} className="bg-white border-2 border-blue-100 rounded-2xl shadow-lg p-6 transition-transform hover:scale-[1.02] hover:shadow-2xl">
              <h2 className="font-bold text-2xl text-blue-700 mb-2 flex items-center gap-2">
                <span className="inline-block bg-blue-100 text-blue-700 px-3 py-1 rounded-full text-xl font-semibold">{course.code}</span>
                {/* <span className="text-gray-700 font-normal text-lg">{course.title}</span> */}
              </h2>
              <ul className="space-y-3 mt-2">
                {course.sections.map((section) => (
                  <li key={section.section} className="flex items-center gap-3 bg-blue-50 rounded-lg p-3 border border-blue-100 hover:bg-blue-100 transition">
        ...
        <div className="w-1/2 sticky top-4 self-start bg-white/90 p-6 border-2 border-purple-200 rounded-2xl shadow-2xl backdrop-blur-lg max-h-[90vh] overflow-y-auto">
          <h2 className="font-bold mb-4 text-xl text-purple-700 flex items-center gap-2">
            <svg xmlns='http://www.w3.org/2000/svg' className='h-6 w-6 text-purple-400' fill='none' viewBox='0 0 24 24' stroke='currentColor'><path strokeLinecap='round' strokeLinejoin='round' strokeWidth={2} d='M9 17v-2a4 4 0 018 0v2m-4-4v4m0 0v4m0-4H5a2 2 0 01-2-2V7a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-7z' /></svg>
            Selected Courses
          </h2>
          {error && <div className="bg-red-100 text-red-700 p-3 mb-4 rounded-lg border border-red-200 shadow">{error}</div>}
          <ul className="space-y-3">
            {selectedSections.length === 0 && (
              <li className="text-gray-400 text-center">No courses selected.</li>
            )}
            {selectedSections.map((section) => (
              <li key={section.id} className="flex items-center gap-3 bg-purple-50 rounded-lg p-3 border border-purple-100 hover:bg-purple-100 transition">
                <div className="flex-1">
                ...
                <button
                  className="bg-gradient-to-r from-red-400 to-pink-500 text-white px-3 py-1 rounded-lg font-bold shadow hover:from-red-500 hover:to-pink-600 transition ml-2"
                  onClick={() => {
          ...
          <button
            className="mt-6 w-full bg-gradient-to-r from-green-400 to-blue-500 text-white py-3 rounded-xl font-bold shadow hover:from-green-500 hover:to-blue-600 transition text-lg"
